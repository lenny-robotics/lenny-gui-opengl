...
template <int Size>
bool InputVectorD(const char* label, Eigen::Matrix<double, Size, 1>& v, const char* format = "%.3f") {
    bool triggered = false;
    if (ImGui::TreeNode(label)) {
        for (int i = 0; i < (int)v.size(); i++) {
            if (ImGui::InputDouble(std::to_string(i).c_str(), &v[i], 0.0, 0.0, format))
                triggered = true;
        }
        ImGui::TreePop();
    }
...
bool SliderVectorD(const char* label, Eigen::Matrix<double, Size, 1>& v, const Eigen::Matrix<double, Size, 1>& v_min,
                   const Eigen::Matrix<double, Size, 1>& v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0) {
    bool triggered = false;
    if (ImGui::TreeNode(label)) {
        for (int i = 0; i < (int)v.size(); i++) {
